...
The transfer component now uses performance above that frozen forecast, weighted by the proportion of incoming players started and averaged per player per Gameweek. It replaces the previous heuristic transfer bonus within the same ±0.8 future-point budget, reaches full weight only after five evaluated batches, and leaves pending/unrated evidence neutral. This avoids rewarding already-projected squad gains twice; results remain noisy evidence rather than proof of managerial skill. Lineup evidence keeps its separate ±0.8 budget, the draft prior fades over ten observed completed rounds, and the total adjustment stays capped at ±2 points. Frozen active-Gameweek forecasts are unchanged. State retains 200 batches per manager; the public view shows the latest 100 reviews. The first collection after deployment activates this schema; four prospective completed rounds are needed before a new transfer can affect the rating.
