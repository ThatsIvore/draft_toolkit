...
4. **LAN-only contract POC implemented:** the separate ephemeral [self-hosted Standard FPL runner](STANDARD_FPL_SELF_HOSTED_RUNNER.md) removes the laptop Python requirement without storing uploads or reports. A published image, Unraid template, remote authentication and optional private history remain separate later slices.
