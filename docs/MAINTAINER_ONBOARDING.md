...
## Paired deadline evaluation (21 September 2026)
...
`selection_trial.py` is evaluation-only. `pending_deadline_selections` stores the latest valid pair separately from immutable `pending_forecasts`; `deadline_selection` and `selection_comparison` follow the existing bounded outcome history. Capture must use actionable fixtures and evaluation must use scoring fixtures and the complete scoring player pool. Partial transfer review confidence uses observed rounds, never the number of collections.
