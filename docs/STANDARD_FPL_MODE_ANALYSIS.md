...
The POC has also been exercised against a live 2026/27 entry without committing its identifier or report. It correctly separated live GW1 facts from GW2–GW5 advice and generated a legal 15-player/11-starter result. A controlled signed-in browser probe has since confirmed that normal Pick Team and Transfers views expose the complete private snapshot allowlist without reading browser credentials or storage. The remaining personal connector work is a fail-closed two-stage DOM exporter, not further authentication reverse engineering.
