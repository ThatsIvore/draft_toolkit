...
The next personal delivery option has now been assessed in [Standard FPL Self-Hosted Runner Discovery](STANDARD_FPL_SELF_HOSTED_RUNNER.md). A separate ephemeral service on the manager's Unraid server can reuse the Python model and let a managed laptop complete the flow through a browser. This is feasible without weakening Draft/H2H isolation, but the private HTTP boundary, container packaging and LAN/remote-access policy are not yet implemented.
