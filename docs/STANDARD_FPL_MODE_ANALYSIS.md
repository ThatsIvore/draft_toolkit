...
The implemented [browser-local private report viewer](STANDARD_FPL_PRIVATE_VIEWER.md) provides a deployable hands-on testing surface without committing or uploading the private report. It does not solve hosted analysis or authentication: the Python report must still be generated locally and selected with the browser file picker.

The first [Standard FPL Self-Hosted Runner](STANDARD_FPL_SELF_HOSTED_RUNNER.md) slice is implemented. Its separate ephemeral HTTP service accepts only the strict sanitized snapshot plus an ordinary public entry URL, reuses the Python model in memory and renders the result in the browser. The Docker image has a health check and no persistent volume. Remote authentication, a published image, an Unraid template and private history remain later work; the first service must stay on the trusted LAN.
