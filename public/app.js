...
function renderActivity() {
  ...
  const outcomes = reviews.length ? `<details class="activity-card"><summary>Transfer reviews (${esc(reviews.length)})</summary><div class="empty">Four completed Gameweeks compare incoming and outgoing player points. Started points show use in the submitted XI, not total team gain. Ratings use acquisition performance above the original forecast, with a ±0.8 point cap. Short reviews carry less weight; started points are tracked separately. Unrated records lack an original forecast or a balanced same-position batch; stopped reviews retain observed results before the player left.</div>${reviews.map(x => {
    const o = x.outcome || {};
