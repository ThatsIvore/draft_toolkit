...
function openPlayer(id) {
  ...
  drawer.innerHTML = `<div class="drawer-head"><button id="drawer-close" aria-label="Close">×</button><div class="eyebrow">${esc(p.position)} · ${esc(p.club || '-')}</div><h2>${esc(p.player)}</h2><div>${esc(p.total_points ?? 0)} points</div></div>
    ...
    <div class="drawer-section"><strong>How v0.3 decides</strong><div class="model-grid"><span>Roster value <b>${esc(intel.roster_score ?? '-')}</b></span><span>Stash value <b>${esc(intel.stash_score ?? '-')}</b></span><span>Future fixtures <b>${esc(intel.future_fixture_score ?? '-')}</b></span><span>Availability <b>${esc(intel.availability_score ?? '-')}</b></span></div><div class="model-note">The action combines ownership state with roster/stash value, health trend and current availability. Explicit return dates are used only when they can be parsed from official FPL news; the toolkit does not invent a recovery date.</div></div></div>`;
