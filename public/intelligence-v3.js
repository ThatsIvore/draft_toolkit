...
function openPlayer(id) {
  ...
  const recent = intel.recent_match_evidence || {};
  const transfer = p.transfer_intel || {};
  const transferSources = (transfer.sources || []).map(source => `<a href="${esc(source.url)}" target="_blank" rel="noopener noreferrer">${esc(source.label || 'Source')}</a>`).join(' · ');
  const transferPanel = transfer.record_id ? `<div class="drawer-section"><strong>Transfer evidence · ${esc(transfer.action || 'WATCH')}</strong><div class="news">${esc(transfer.summary || '')}</div><div class="model-grid"><span>Destination <b>${esc(transfer.destination?.club || '-')}</b></span><span>Status <b>${esc(String(transfer.status || '').replaceAll('_', ' '))}</b></span><span>Destination fixtures <b>${esc(transfer.destination_fixture_score ?? '-')}</b></span><span>Fixture change <b>${transfer.fixture_delta == null ? '-' : `${Number(transfer.fixture_delta) > 0 ? '+' : ''}${esc(transfer.fixture_delta)}`}</b></span></div><div class="model-note">${transferSources} · expires ${esc(transfer.expires_at || '-')}</div></div>` : '';
  const recentGameweeks = (recent.gameweeks || []).map(gw => `<span>GW${esc(gw.gameweek)} <b>${gw.grade ? esc(gw.grade) : 'No appearance'}</b>${gw.grade_score == null ? '' : ` · ${esc(gw.grade_score)}`}</span>`).join('');
  drawer.innerHTML = `<div class="drawer-head"><button id="drawer-close" aria-label="Close">×</button><div class="eyebrow">${esc(p.position)} · ${esc(p.club || '-')}</div><h2>${esc(p.player)}</h2><div>${esc(p.total_points ?? 0)} points</div></div>
    ...
    ${transferPanel}
    <div class="drawer-section"><strong>Recent completed-Gameweek evidence</strong><div class="model-grid"><span>Grade <b>${esc(recent.grade || '-')}</b></span><span>Score <b>${recent.status === 'available' ? esc(recent.score) : '-'}</b></span><span>Confidence <b>${recent.status === 'available' ? `${esc(recent.confidence)}%` : '-'}</b></span><span>Model adjustment <b>${recent.status === 'available' ? esc(recent.adjustment) : '0'}</b></span>${recentGameweeks}</div><div class="model-note">Only final, officially checked Gameweeks count. Grades compare players with their own position; double Gameweeks are aggregated. Influence is capped at ±5 model points.</div></div>
    <div class="drawer-section"><strong>How v0.6.0 decides</strong><div class="model-grid"><span>Roster value <b>${esc(intel.roster_score ?? '-')}</b></span><span>Stash value <b>${esc(intel.stash_score ?? '-')}</b></span><span>Post-return fixtures <b>${esc(intel.post_return_fixture_score ?? '-')}</b></span><span>Availability <b>${esc(intel.availability_score ?? '-')}</b></span></div><div class="model-note">The stash case uses fixtures from the estimated return Gameweek onward when official FPL news provides a readable date. Fixtures before that return are not credited, and the toolkit never invents a recovery date.</div></div></div>`;
