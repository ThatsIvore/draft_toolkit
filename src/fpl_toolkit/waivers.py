...
def attach_replacement_analysis(
    ...
    v0.5 adds floor/upside deltas and sample confidence. Pre-GW1 recommendations
    are intentionally more conservative because role and usage certainty are lower.
    """
    ...
    for available in available_players:
        ...
        if combined >= swap_threshold and immediate_delta >= 2 and floor_delta >= 0 and confidence != "LOW":
            action = "SWAP NOW"
        elif combined >= (10.0 if preseason else 7.0) and future_delta > immediate_delta and upside_delta > 0 and confidence != "LOW":
            action = "STASH SWAP"
